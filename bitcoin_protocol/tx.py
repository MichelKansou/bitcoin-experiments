...
class Tx:
    ...
    def is_coinbase(self):
        ...
        coinbase_in_tx = self.tx_ins[0]
        # check that first input prev_tx is b'\x00' * 32 bytes
        if coinbase_in_tx.prev_tx != b'\x00'*32:
            return False
        # check that first input prev_index is 0xffffffff
        if coinbase_in_tx.prev_index != 0xffffffff:
            return False
        return True
    ...
    def coinbase_height(self):
        ...
        # grab the first cmd
        coinbase_in_tx = self.tx_ins[0]
        coinbase_script_sig = coinbase_in_tx.script_sig
        coinbase_script_cmd = coinbase_script_sig.cmds[0]
        # convert the cmd from little endian to int
        return little_endian_to_int(coinbase_script_cmd)
